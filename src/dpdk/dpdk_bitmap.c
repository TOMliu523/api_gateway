...
struct dpdk_bitmap *dpdk_bitmap_init(uint32_t bit_count, int hw_numa_id)
{
    ...
    return rte_bitmap_init(bit_count, bitmap, size);
}

struct dpdk_bitmap *dpdk_bitmap_init_with_all_set(uint32_t bit_count, int hw_numa_id)
{
    uint32_t size = 0;
    uint8_t *bitmap = NULL;
    ...
    return rte_bitmap_init_with_all_set(bit_count, bitmap, size);
}
